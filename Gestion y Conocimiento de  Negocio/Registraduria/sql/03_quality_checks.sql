...
WITH fact_dates AS (
    SELECT
        ...
        (de.full_date - ds.full_date + 1) AS calendar_days
...
SELECT
    year_num,
    COUNT(*) FILTER (WHERE start_date > end_date) AS bad_date_order,
    COUNT(*) FILTER (WHERE ABS(commission_days - calendar_days::NUMERIC) > 0.50) AS day_mismatch_rows
FROM fact_dates
GROUP BY year_num
ORDER BY year_num;
