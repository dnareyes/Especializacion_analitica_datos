-- Viaticos DW - starter analytics queries for BigQuery
...
JOIN viaticos_dw.dim_tiempo t ON t.date_key = f.start_date_key
...
FROM viaticos_dw.fact_viaticos f
JOIN viaticos_dw.dim_empleado e ON e.employee_key = f.employee_key
...
SELECT
  ...
  AVG(f.total_travel_allowance) AS avg_allowance
FROM viaticos_dw.fact_viaticos f
JOIN viaticos_dw.dim_ruta r ON r.route_key = f.route_key
...
SELECT
  ...
  SUM(f.total_travel_allowance) AS total_allowance,
  SUM(f.commission_days) AS total_days,
  SAFE_DIVIDE(SUM(f.total_travel_allowance), NULLIF(SUM(f.commission_days), 0)) AS allowance_per_day
FROM viaticos_dw.fact_viaticos f
JOIN viaticos_dw.dim_procedimiento p ON p.procedure_key = f.procedure_key
...
SELECT
  ...
  SAFE_DIVIDE(f.total_travel_allowance, NULLIF(f.commission_days, 0)) AS allowance_per_day
FROM viaticos_dw.fact_viaticos f
JOIN viaticos_dw.dim_empleado e ON e.employee_key = f.employee_key
JOIN viaticos_dw.dim_procedimiento p ON p.procedure_key = f.procedure_key
JOIN viaticos_dw.dim_ruta r ON r.route_key = f.route_key
ORDER BY allowance_per_day DESC
LIMIT 100;
